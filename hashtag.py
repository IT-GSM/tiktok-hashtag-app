    ...
    async with TikTokApi() as api:        
        ...
        with app.app.app_context():
            ...
            for hashtag in hash_out_data:
                ...
                #         await asyncio.sleep(3)                 
                #         app.db.session.commit()
                #         print("Updated hashtag {}".format(hash_video_id))
...
if __name__ == "__main__":
    ...
    random.shuffle(all_hashtag)
    sources = [''.join(user) for user in all_hashtag] 
    print(sources)
    sources = ["NLD"]

    loop = asyncio.get_event_loop()
    loop.run_until_complete((get_hashtag_videos(sources)))
